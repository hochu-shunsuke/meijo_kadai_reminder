...
function dailySystemRun() {
  ...
  try {
    ...
    // UIアラートはトリガー実行時には出ないが、手動実行時用に残す
...
function fetchWebClassAssignments() {
  ...
  courseLinks.forEach((course, index) => {
    ...
    // ★★★★ 修正点: course.nameから無駄な情報を除去する ★★★★
    let cleanedCourseName = course.name;
    // 1. 先頭の数字（桁数不明）とその後のスペースを除去 (例: '642816 ')
    cleanedCourseName = cleanedCourseName.replace(/^\s*\d+\s*/, '').trim(); 
    // 2. 末尾の括弧とその内容（例: ' (2025-後期-火1-他)'）と、それに先行するスペースを除去
    cleanedCourseName = cleanedCourseName.replace(/\s*\(.*\)\s*$/, '').trim();
    // ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
    
    const courseLogMessage = `[${index + 1}/${courseLinks.length}] 科目: ${cleanedCourseName} | URL: ${course.url} を取得中...`; // ログも修正
    ...
    try {
      ...
      const successMessage = `   -> HTML取得成功。サイズ: ${htmlSize} バイト。`;
      ...
      assignments.forEach(assignment => {
        ...
        allAssignmentRows.push([
          'WebClass',                   // ソース (0)
          cleanedCourseName,            // 授業名 (1) <-- 修正した科目名を使用
      ...
      Logger.log(`🚨 科目「${cleanedCourseName}」の課題取得中にエラーが発生しました: ${e.message}。この科目をスキップします。`); // ログも修正
      logToSheet(`🚨 科目「${cleanedCourseName}」の課題取得中にエラーが発生しました: ${e.message}。`); // ログも修正
    }
  ...
  sheetNames.forEach(sheetName => {
    ...
    data.forEach((row, index) => {
      ...
      if (!tasksId && registeredFlag !== 'COMPLETED' && registeredFlag !== 'DELETED' && registeredFlag !== 'EXPIRED') {
        ...
        let dueDateDisplayString = ''; // メモに表示するための整形済み文字列
        
        // 1. スプレッドシートの値が日付オブジェクトとして取得された場合
        if (dueDate instanceof Date) {
            dueDateTime = dueDate;
            // Notes用に「yyyy/MM/dd HH:mm」形式に再フォーマット
            dueDateDisplayString = Utilities.formatDate(dueDate, Session.getScriptTimeZone(), 'yyyy/MM/dd HH:mm');
        } 
        // 2. スプレッドシートの値が文字列として取得された場合
        else if (typeof dueDate === 'string' && dueDate.trim().length > 0) {
            dueDateDisplayString = dueDate; // 整形済みの文字列をそのまま使用
            
            // Tasks API登録用のDateオブジェクトを作成
            const dateStr = dueDate.replace(/\//g, '-');
            dueDateTime = new Date(dateStr);
        }
        
        // 日付が無効ならスキップ
        if (dueDateTime && dueDateTime.toString() === 'Invalid Date') {
            Logger.log(`🚨 無効な日付形式を検出 (${sheetName}, ${title}): ${dueDate}`);
            return; 
        }

        // 期限が過去のものかチェック
        if (dueDateTime && dueDateTime.getTime() < new Date().getTime()) {
            row[7] = 'EXPIRED'; 
            updated = true;
            return;
        }
        ...
        try {
          ...
          if (dueDateTime) {
            // 時刻を無視して日付のみを取得する処理（Task APIの終日タスク用）
            const dayOnly = new Date(Date.UTC(dueDateTime.getFullYear(), dueDateTime.getMonth(), dueDateTime.getDate()));
            task.due = dayOnly.toISOString();
          }
          
          // リンク (Notesとして追加)
          // メモには、取得した正確な日時（dueDateDisplayString）を記載する
          task.notes = `課題リンク:\n${link}\n\nソース: ${source}${startDate ? `\n開始日時: ${startDate}` : ''}${dueDateDisplayString ? `\n終了日時/締切: ${dueDateDisplayString}` : ''}`;

          // Tasks APIでタスクを挿入 (登録)
          const insertedTask = Tasks.Tasks.insert(task, taskListId);
          
          // スプレッドシートの該当行を更新
          data[index][6] = insertedTask.id; 
          data[index][7] = 'REGISTERED';   
          updated = true;
